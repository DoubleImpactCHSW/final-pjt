<template>
    <div>
        ...
        <select id="country" v-model="selectedCountry" @change="calculateExchangeRate">
            <option v-for=" country in rateInfo" :key="country.cur_unit" :value="country.cur_nm">{{ country.cur_nm }}</option>
        </select>

        <br>

        <label for="foreign">{{ selectedCountry }}</label>
        <input id="foreign" type="number" v-model="foreignValue" @input="calculateExchangeRate">

        <br>

        <label for="korea">원화 가치</label>
        <input id="korea" type="number" v-model="wonValue" @input="calculateExchangeRateReverse">
    </div>  
</template>
...
export default {
    ...
    methods: {
        calculateExchangeRate() {
            const bkpr = this.rateInfo.filter((c) => c.cur_nm === this.selectedCountry)[0].bkpr
